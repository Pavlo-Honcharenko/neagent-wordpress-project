<?php
/**
 *  KUB REALTY ESTATE Importer Script (INSERT ONLY)
 */
 ...
 */
class Partner_Feed {
    private $feed_url = 'https://kubrealtyestate.ligapro.ua/files/yandex_xml/ya_xml_ce016_05292.xml'; // ======= ВСТАВИТИ URL
...
class Partner_Importer { 
    ...
    private function log($msg) {
        ...
        // Log to file
        file_put_contents(
            __DIR__ . '/import_kubrealtyestate.log', 
    ...
    public function run( $source = '' ) {
        ...
        $max_to_process   = 100; // Limit how many ads from the feed will be checked
        
        $imported           = 0;
        $processed_this_run = 0; // Counter of viewed items

        // --- LOGIC OF DISPLACEMENT (OFFSET) ---
        $current_offset = (int)get_option('kubrealtyestate_import_offset', 0); 
        ...
        for ($i = $current_offset; $i < $total; $i++) {
            ...
            $type_offer   = strtolower( trim( (string) $item->type ) );
            $type_realty  = strtolower( trim( (string) $item->category ) );
            $map_key      = $type_offer . '_' . $type_realty;
            $category_slug = isset( $category_map[ $map_key ] ) ? $category_map[ $map_key ] : '';

            $cat_term = $category_slug ? get_term_by( 'slug', $category_slug, 'hp_listing_category' ) : false;
            if ( ! $cat_term ) {
            ...
            if ( ! $city_term ) {
                $this->log("The object $external_id is not added...");
                continue;
            }
            ...
            if ($ids) {
                ...
                // Створюємо серіалізований масив для HivePress
        ...
        update_option('kubrealtyestate_import_offset', $i >= $total ? 0 : $i);

        $final_offset = get_option('kubrealtyestate_import_offset');
        $this->log("Cycle finished. Next offset: $final_offset. Processed: $processed_this_run, Imported: $imported");
    }
}

(new Partner_Importer())->run();
